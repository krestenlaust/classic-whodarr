...
class WebArchiveSerialFilenameConverter(episodeRecognizer: EpisodeRecognizer)
  ...
  override def convertEpisodeFilename(filename: String): String =
    ...
    if episodeId.isEmpty then sanitizedFilename
    else
      sanitizedFilename.replace(
        serialDesignation,
        formatEpisodeDesignation(episodeId.get)
      )
